...
<div class="container border mx-3 mb-3 py-2 rounded" id="filter">
		...
		<div class="row mb-2">
			...
			<div class="col-md-2">
				...
				<select class="form-select" id="test-f2-select" name="f2">
					...
					<c:forEach var="num" items="${class_num_set}">
						<option value="${num}" <c:if test="${class_num==f2}">selected</c:if>>${num}</option>
					</c:forEach>
				</select>
			</div>
			...
			<div class="col-md-4">
				...
				<select class="form-select" id="test-f3-select" name="f3">
					<option value="0">--------</option>
					<c:forEach var="num" items="${class_num_set}">
						<option value="${num}" <c:if test="${subject==f2}">selected</c:if>>${num}</option>
					</c:forEach>
				</select>
			</div>
	...
	<!-- 学生情報フォーム -->
	<form action="TestListSubjectExcecute.action?f4=${f4}&f=${f}" method="get">
		...
		<div class="row mb-2">
			...
			<div class="col-md-4">
				...
				<input type="text" class="form-control" id="test-f4-select" name="f4" required>
			</div>
